 ...
 *  any failure (network, malformed payload, missing endpoint) returns an
 *  empty list — the globe simply skips the chokepoint layer. There's no
 *  retry; the data is refreshed on the next app session. */
export function useChokepoints(): { chokepoints: Chokepoint[]; loading: boolean } {

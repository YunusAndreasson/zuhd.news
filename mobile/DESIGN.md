...
- **Two surfaces, and each answers a different question.**
  - **The bar** (`MapHeader`) — one row above the earth: the zuhd mark fixed at the left (the name is not spelled out), the gauges (`IndicatorStrip`) scrolling sideways from beside it to the screen's edge, and the menu as the last item in that scroll after `all ›` (alone on the right before the gauges load). The gauges: subject, reading, move, for every market, strait and currency that moved, largest move first. *How much.* Slots 3.4 across so the cut one says the row continues, each subject on one line and never cut (a strait prints `Hormuz Str.`; a longer name widens its slot); no marquee, because a ticker moves when nothing has happened, which is an engagement mechanic.
  - **The sheet** (`MapSheet`) — news only, one story at a time (`StoryDeck` → `StoryCard`), swiped sideways through the strictly newest-first river. At rest the card is its kicker, `title` and lead (the hook and why-it-matters sentences); pulled up it is the whole story, with the earth scaled into a band above it. *What, and what happened.* Above the card, one masthead row: a round listen button (filled, so it reads as its own control and not the track's play head), then a track that fills as the deck moves and a list icon, which open the `IndexSheet` of every story — or, while one is live, a Red hazard alert. No digits: the track is the status and the icon the signifier.

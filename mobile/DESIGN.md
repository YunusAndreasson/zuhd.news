...
- **Two surfaces, and each answers a different question.**
  - **The bar** (`MapHeader`) — one row above the earth: the zuhd mark at the left, which is home (newest story, zoom released, gauges back to their start); the gauges (`IndicatorStrip`) scrolling sideways from beside it to the screen's edge, over a shade of the screen's own ground that fades out below the row so the labels read over lit land and city lights. Listen is on the sheet's masthead, beside the story list. The name is not spelled out. Settings and pages open from the cog beside the briefing button on the sheet masthead. The gauges: subject, reading, the move over the past seven days and the week's line (`Sparkline`, coloured by the move's valence), for every market, strait and currency with a week to show, largest move first. *How much, and how.* One window for every slot, named once as `7 days` over `all` at the row's end, because a row sorted by size is only a comparison if every slot measures the same thing; the card a slot opens keeps its own window and prints it. The slot whose card is open carries a 2pt emphasis bar, and the globe rings its place. Slots 3.4 across so the cut one says the row continues, each subject on one line and never cut (a strait prints `Hormuz Str.`; a longer name widens its slot); no marquee, because a ticker moves when nothing has happened, which is an engagement mechanic.
  - **The sheet** (`MapSheet`) — news only, one story at a time (`StoryDeck` → `StoryCard`), swiped sideways through the strictly newest-first river. At rest the card is its kicker, `title` and lead (the hook and why-it-matters sentences); pulled up it is the whole story, with the earth scaled into a band above it. *What, and what happened.* Above the card, one masthead row: a segmented track (one segment per story, continuous past 60) that fills as the deck moves and scrubs like the briefing player's (`useScrub` + `ScrubBar`), and a tappable story count that opens the `IndexSheet` of every story. A live Red hazard alert replaces the track, keeping the count available. The committed position is printed beside the track (`18 of 38`), centered on the same row as the icons; the end card keeps the count at the total. The count and scrubber have 48pt-high touch areas; “Show all articles” is an accessibility hint, with no additional visible label. While scrubbing, a tooltip previews the destination and its time (`5h ago`). Each segment is its story's category hue (the globe's beacons, the card's dot); stories already passed take that hue a solid step quieter (`mixHex` toward the sheet, never an opacity), as found places dim on the globe, and the story on the card keeps its full hue. The river stays in time order: colour shows the day's mix without regrouping it, and a category jump belongs in the index sheet as a filter. Nothing that plays sits on the track — a play button beside a progress bar reads as its play head. Listen and the settings cog are icons with separate 40×48pt touch areas in a compact group: a paused briefing draws how much is heard as an arc round its button, their touch areas meet without overlapping hit slop, and a one-time hint pill teaches both after the globe lesson.

...
- **The earth reads as a sphere, not a map in a circle.** Zooming makes the planet larger — past the screen's edges when it outgrows them — rather than magnifying the ground inside a fixed disc; the horizon is always the real limb. The web's 30° graticule sits under the land, and the day side is lifted in `daylight` so night is visible over the sea, not only on the continents.
...
- Shadows, gradients, box-shadows (the `BriefingBar` iOS frosted-glass is the one carve-out — see §Native chrome carve-outs).

...
ink step on its kicker, and landing on that card is the caught-up moment. A card whose content changed since the reader last viewed it
...
- **Two surfaces, and each answers a different question.**
  - **The bar** (`MapHeader`) — one row above the earth: the gauges (`IndicatorStrip`) scrolling sideways from the left inset to a fixed Settings cog at the top right, over a shade of the screen's own ground that fades out below the row so the labels read over lit land and city lights. Listen is on the sheet's masthead, beside the story scrubber. Its button shares the Settings button's centre line at the safe right edge. The name is not spelled out. Settings and pages open from the fixed top-right cog, which remains available when the gauges recede. Settings uses default ink for clear contrast. The gauges: subject, direction glyph and percentage move on one line (absolute readings and graphs stay in the detail sheet), for every market, strait and currency with a week to show, largest move first. *How much, and how.* One window for every slot, named once as `7 days` at the row's start, because a row sorted by size is only a comparison if every slot measures the same thing; the card a slot opens keeps its own window and prints it. The slot whose card is open carries a 2pt emphasis bar, and the globe rings its place. Slots grow to fit their label and move without wrapping (a strait prints `Hormuz Str.`); a partial slot at the edge signals that the row continues; no marquee, because a ticker moves when nothing has happened, which is an engagement mechanic.
  - **The sheet** (`MapSheet`) — news only, one story at a time (`StoryDeck` → `StoryCard`), swiped sideways through category groups, newest first within each group. At rest the card is its kicker, `title` and lead (the hook and why-it-matters sentences); pulled up it is the whole story, with the earth scaled into a band above it. *What, and what happened.* Above the card, one masthead row: a segmented track (one segment per story, continuous past 60) that fills as the deck moves and scrubs like the briefing player's (`useScrub` + `ScrubBar`), with Play beside it. There is no separate all-articles list. A live Red hazard alert replaces the track and opens its details when pressed. The active segment marks the current article; its numeric position remains in the scrub tooltip and accessibility label. The scrubber and Play have 48pt-high touch areas. While scrubbing, a tooltip previews the destination and its time (`5h ago`). Each segment is its story's category hue (the globe's beacons, the card's dot); stories already passed take that hue a solid step quieter (`mixHex` toward the sheet, never an opacity), as found places dim on the globe, and the story on the card keeps its full hue. The river groups politics, economy, science, then tech, newest first within each category: contiguous color bands let the reader scrub directly to a category. Nothing that plays sits on the track — a play button beside a progress bar reads as its play head. Listen has a 40×48pt touch area beside the scrubber; Settings lives in the top bar: a paused briefing draws how much is heard as an arc round its button, its touch area does not overlap the scrubber, and a one-time hint pill teaches the masthead after the globe lesson.
...
- **Information appears exactly once.** The strip prints instruments; the sheet prints news; alerts never enter the deck. A contract reaches a story only as the odds line on the grown card. The scrubber provides category previews and direct jumps through the same deck.
...
- **Overflow scrolls; it must never truncate.** No card text clamps: a long title or lead runs on under the fold at rest and scrolls when grown. The only single-line truncation is metadata (kickers, meta lines).

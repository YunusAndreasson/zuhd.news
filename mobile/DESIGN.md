...
The deck marks the first story the reader has already seen with an `earlier`
ink step on its kicker, and landing on that card is the caught-up moment; the
index sheet's rows carry the same ink step. A card whose content changed since the reader last viewed it
...
One typeface family. Whitespace is designed. Color carries meaning only — every non-monochrome element must justify its hue. No shadows, no gradients, no decorative icons — the one material carve-out is the `BriefingBar`'s iOS-only frosted glass (see §Native chrome carve-outs). Restraint is the brand.
...
- **One earth, and every surface is a layer over it.** `MiniGlobe` is mounted once at the screen root; the bar and the sheet sit above the same canvas. Stories are read on the map, never over it: there is no full-screen reader, because covering the earth to read a 350-character story lost the one thing this screen is for.
- **Two surfaces, and each answers a different question.**
  - **The bar** (`MapHeader`) — one row above the earth: the zuhd mark at the left, which is home (newest story, zoom released, gauges back to their start); the gauges (`IndicatorStrip`) scrolling sideways from beside it to the screen's edge, over a shade of the screen's own ground that fades out below the row so the labels read over lit land and city lights. Listen is on the sheet's masthead, beside the story list. The name is not spelled out. Settings and pages open from the trailing button in the story list's header. The gauges: subject, reading, the move over the past seven days and the week's line (`Sparkline`, coloured by the move's valence), for every market, strait and currency with a week to show, largest move first. *How much, and how.* One window for every slot, named once as `7 days` over `all` at the row's end, because a row sorted by size is only a comparison if every slot measures the same thing; the card a slot opens keeps its own window and prints it. The slot whose card is open carries a 2pt emphasis bar, and the globe rings its place. Slots 3.4 across so the cut one says the row continues, each subject on one line and never cut (a strait prints `Hormuz Str.`; a longer name widens its slot); no marquee, because a ticker moves when nothing has happened, which is an engagement mechanic.
  - **The sheet** (`MapSheet`) — news only, one story at a time (`StoryDeck` → `StoryCard`), swiped sideways through the strictly newest-first river. At rest the card is its kicker, `title` and lead (the hook and why-it-matters sentences); pulled up it is the whole story, with the earth scaled into a band above it. *What, and what happened.* Above the card, one masthead row: a segmented track (one segment per story, continuous past 60) that fills as the deck moves and scrubs like the briefing player's (`useScrub` + `ScrubBar`), and a list button that opens the `IndexSheet` of every story — or, while one is live, a Red hazard alert. No digits at rest: the track is the status, the segments map a swipe to a story, and `12 of 48` appears only while scrubbing, with the story's time (`5h ago`) as a quieter line under it, raised clear of the thumb on a hairline down to the finger's place. Each segment is its story's category hue (the globe's beacons, the card's dot); stories already passed take that hue a solid step quieter (`mixHex` toward the sheet, never an opacity), as found places dim on the globe, and the story on the card keeps its full hue. The river stays in time order: colour shows the day's mix without regrouping it, and a category jump belongs in the index sheet as a filter. Nothing that plays sits on the track — a play button beside a progress bar reads as its play head. Listen and the list are icons with no words, because words would take a quarter of the track on a small phone: a paused briefing draws how much is heard as an arc round its button, the list is named "today's stories" to a screen reader (the sheet it opens is "today"), the two buttons' touch areas meet between them rather than overlapping, and a one-time hint pill teaches both after the globe lesson.
- **One day of news.** The river, and so the deck, the globe's lights and the index, is the last 24 hours (anchored on the newest story if the desk has not published in a day). An older story appears only when a reader asks for it by name — a saved story, a notification.
- **Information appears exactly once.** The strip prints instruments; the sheet prints news; alerts never enter the deck. A contract reaches a story only as the odds line on the grown card (a bare percentage on its index row). The index sheet is a second view of the same river for scanning and jumping, not a second set of facts.
- **The resting card is sized from type, not a fraction of the window.** `lib/deck-layout.ts` fits the masthead, kicker, two title lines and five lead lines at the reader's font scale, keeps the globe at least 34% of the window (140pt absolute), and is computed once per window and font scale — never per card, which would move the globe on every swipe. What does not fit runs on under the fold; growing reveals it. Grown, the sheet stops at the story's own height (never taller than leaves the globe 20%), and the earth takes the rest — a short story is not a third of a screen of blank sheet.
...
- **The globe's marks are the web map's, glyph for glyph and hue for hue.** Shape says what a mark is; its colour is the web's colour for that thing (`mark*` tokens in `constants/theme.ts`, copied verbatim from `public/islands/_map/style.ts`, identical in both themes because the web map has one palette). Stories are beacons in their **category** hue — politics, economy, science, tech — sized by coverage and faded by age, with the web's contested ring; straits are two facing coastlines in slate, gold when pinched and teal when surging; exchanges are a candle in sage or clay by direction; GDACS hazards are their pictograms in amber; conflict is a red glow; famine is the IPC column in violet; thermal anomalies are a burst in orange; a UN genocide determination is a dark disc with a red ring and an always-on label. This is a deliberate exception to "hierarchy through type, not colour": on the globe, colour *is* the legend, and a phone map that disagreed with the website about what a mark is would be two maps. A new layer takes its silhouette and hue from the web; it does not invent either.
- **A story is found by opening it, and a found place is dimmed, never erased.** Tapping a beacon plays a burst in its hue and jumps the deck to that story's card; once every story at a place is found, its beacon becomes a small hollow ring in the same hue, which reopens the newest story when tapped. A thin ring just outside the globe is what is left to find, shrinking back toward twelve o'clock after each burst — no number anywhere on screen: the ring is the status, and the exact count is spoken by the masthead's accessibility label. Growing a card, or swiping onto one while grown, counts too; swiping past a card at rest does not. Found index rows drop to secondary ink. State lives in `lib/found-store.ts`, places in `lib/story-places.ts`.
- **Gesture ownership is spatial, and each axis has one job.** On the globe at rest: a drag turns it (and a release glides), a pinch zooms about the fingers, a tap hit-tests; with a story grown, a tap puts it down. On the bar: sideways scrolls instruments. On the sheet: sideways is the river (next story left, newer right), vertical is depth — pull up to read, pull down to put the story back, pull down at rest to refresh — and a grown card scrolls. The deck's pan and the sheet's pan are never simultaneous, so a drag is a swipe or a sheet move, never half of each.
...
- **Overflow scrolls; it must never truncate.** No card text clamps: a long title or lead runs on under the fold at rest and scrolls when grown. Index rows take their natural height. The only single-line truncation is metadata (kickers, meta lines).
...
- **Every mark on the globe has a row somewhere, because the globe is hidden from screen readers.** VoiceOver activates an element at its geometric centre, which on a globe is a lottery country. The bar's gauges, the index sheet and the instruments sheet are the accessible path to everything the earth shows; a mark layer without one is an accessibility regression. The story card's swipe is reachable as its `next story` / `previous story` accessibility actions.
...
- Shadows, gradients, box-shadows (the `BriefingBar` iOS frosted-glass and the top bar's shade are the carve-outs — see §Native chrome carve-outs).
...
The "no native chrome" rule has three specific, intentional carve-outs:
...
- **The top bar sits on a shade.** `MapHeader` floats over the globe, and caps gauge labels over a lit coastline were hard to read. Behind it, one vertical gradient of `bg` — strongest under the status bar, still holding under the row, gone `SPACING.xl` below it — so the earth runs up into the bar instead of stopping at an edge. It is a legibility scrim, the web map's HUD answer, not decoration; nothing else in the chrome gets a gradient.

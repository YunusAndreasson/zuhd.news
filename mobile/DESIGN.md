...
- **Two surfaces, and each answers a different question.**
  - **The bar** (`MapHeader`) — one row above the earth: the gauges (`IndicatorStrip`) scrolling sideways from the left inset to a fixed Settings cog at the top right, over a shade of the screen's own ground that fades out below the row so the labels read over lit land and city lights. Listen is on the sheet's masthead, beside the story list. The name is not spelled out. Settings and pages open from the fixed top-right cog, which remains available when the gauges recede. Settings and All articles use default ink for clear contrast. The gauges: subject, reading, the move over the past seven days (graphs stay in the bottom sheet), for every market, strait and currency with a week to show, largest move first. *How much, and how.* One window for every slot, named once as `7 days` over `all` at the row's end, because a row sorted by size is only a comparison if every slot measures the same thing; the card a slot opens keeps its own window and prints it. The slot whose card is open carries a 2pt emphasis bar, and the globe rings its place. Slots 3.4 across so the cut one says the row continues, each subject on one line and never cut (a strait prints `Hormuz Str.`; a longer name widens its slot); no marquee, because a ticker moves when nothing has happened, which is an engagement mechanic.
  - **The sheet** (`MapSheet`) — news only, one story at a time (`StoryDeck` → `StoryCard`), swiped sideways through the strictly newest-first river. At rest the card is its kicker, `title` and lead (the hook and why-it-matters sentences); pulled up it is the whole story, with the earth scaled into a band above it. *What, and what happened.* Above the card, one masthead row: a segmented track (one segment per story, continuous past 60) that fills as the deck moves and scrubs like the briefing player's (`useScrub` + `ScrubBar`), and an All articles list icon to the right of Play that opens the `IndexSheet` of every story. A live Red hazard alert replaces the track, keeping All articles available. The active segment marks the current article; its numeric position remains in the scrub tooltip and accessibility label. The scrubber and both action icons have 48pt-high touch areas. While scrubbing, a tooltip previews the destination and its time (`5h ago`). Each segment is its story's category hue (the globe's beacons, the card's dot); stories already passed take that hue a solid step quieter (`mixHex` toward the sheet, never an opacity), as found places dim on the globe, and the story on the card keeps its full hue. The river stays in time order: colour shows the day's mix without regrouping it, and a category jump belongs in the index sheet as a filter. Nothing that plays sits on the track — a play button beside a progress bar reads as its play head. Listen has a 40×48pt touch area beside the scrubber; Settings lives in the top bar: a paused briefing draws how much is heard as an arc round its button, its touch area does not overlap the scrubber, and a one-time hint pill teaches the masthead after the globe lesson.

...
- **One earth, and every surface is a layer over it.** `MiniGlobe` is mounted once at the screen root; the strip and the sheet sit above the same canvas. Stories are read on the map, never over it: there is no full-screen reader, because covering the earth to read a 350-character story lost the one thing this screen is for.

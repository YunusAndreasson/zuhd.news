...
- **Three surfaces, and each answers a different question.**
  ...
  - **The sheet** (`MapSheet`) — news only, no masthead: a NOW block that appears only for a live Red hazard alert, then the strictly newest-first river. *What.* No straits, markets, currencies or contracts — those are the strip's, and `all →` at its end opens the full list.
  - **The reader** (`ReaderLayer`) — the article pager, unchanged. *What happened, in full.*
- **Information appears exactly once across those three.** The strip prints instruments; the sheet prints news; the river's first row is the lead story, so the NOW block holds no stories at all. A contract reaches the sheet only as the odds chip on the story it settles.
...
- **A story is found by opening it, and a found story leaves the globe.** Tapping a beacon plays a burst in its hue, drops the mark, and opens the story in the sheet as a preview (`StoryPreview`) — kicker with the category dot, title, `read story`, then the lead. The masthead counts `found N of M`. Opening a story from its row or paging to it in the reader counts too; found rows drop to secondary ink. State lives in `lib/found-store.ts`, places in `lib/story-places.ts`.
...
- **Every mark on the globe has a row somewhere, because the globe is hidden from screen readers.** VoiceOver activates an element at its geometric centre, which on a globe is a lottery country. The strip, the alert block and the instruments sheet are the accessible path to everything the earth shows; a mark layer without one is an accessibility regression.

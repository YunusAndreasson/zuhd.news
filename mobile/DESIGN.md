...
- **Two surfaces, and each answers a different question.**
  ...
  - **The dock** (`StoryDock`) — one row pinned to the foot of the screen, not to the sheet, so it is in the same place at rest and open: `(‹ 3 new) [story track] (⌃) (›)`. It used to be a masthead on top of the sheet — mid-screen at rest, near the top with a story open, where no thumb holding the phone reaches. The track is a segmented bar (one segment per story, continuous past 60) that fills as the deck moves and scrubs like the briefing player's (`useScrub` + `ScrubBar`); each segment is its story's category hue (the globe's beacons, the card's dot), stories already passed a solid step quieter (`mixHex` toward the sheet, never an opacity). The newest story is at the track's left end, and stories new since the reader last had the feed carry a 2pt rule in emphasis ink over their segments, so a glance at the dock says whether anything arrived. When new stories the reader has not had sit behind the one in front — a refresh put them ahead of where they were reading, or a scrub skipped them — a `‹ 3 new` pill in the circles' own material leads the row and jumps to the newest of them; it is ink, not a badge, and it goes when nothing is left behind the reader. While scrubbing, a tooltip previews the destination and its time (`5h ago`). A live Red hazard alert replaces the track and opens its details when pressed. `⌃` opens the story on the card and closes it again — a chevron that turns with the sheet, up at rest and down open, so it always points where the sheet would go; before it, closing a story meant a pull down the sheet or a tap on the globe at the top of the screen. `›` is the next story, in the corner because it is the control used forty times a session; it slides the deck exactly as a swipe does (`StoryDeck.step`), and on the end card it drops its fill. Every target is 48pt square. **The buttons are one shape**: a 40pt `pillBg` circle each with a 20pt glyph, 8pt apart, the group standing 16pt off the track — twice the buttons' own gap, so position and actions read as two groups — and the last circle as far from the screen's edge as the track's start. They were a 28pt circle, a bare chevron and a 40pt circle, three treatments for one kind of control; `›` is marked by emphasis ink, not size. The briefing's `▶` was the first circle until 2026-09-21; it is in the top bar now. A button with nothing to do drops its fill. The briefing player bar sits on the dock rather than over it, and bottom toasts end above it.

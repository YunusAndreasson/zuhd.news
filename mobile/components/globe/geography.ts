 ...
 * Every layer is an `OrthoLayer`: its vertices' unit vectors, computed here
 * once, are what a frame rotates and clips (`ortho-stream.ts`). */
export function getGlobeGeography(tier: GeographyTier): GlobeGeography {
  ...
  const result: GlobeGeography = {
    land: createOrthoLayer(feature(topology, topology.objects.land)),
    borders: createOrthoLayer(mesh(topology, topology.objects.countries, (a, b) => a !== b)),
    ice: createOrthoLayer({

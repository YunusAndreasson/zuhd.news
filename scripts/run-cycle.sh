#!/usr/bin/env bash
# zuhd.news editorial cycle — runs every 3 hours via systemd timer
...
timeout 300 claude --dangerously-skip-permissions --model claude-opus-4-6 --effort high -p "$SELECT_PROMPT" 2>&1 | tee -a "$LOG_FILE"
SELECT_EXIT=$?
...
timeout 600 claude --dangerously-skip-permissions --model claude-opus-4-6 -p "$WRITE_PROMPT" 2>&1 | tee -a "$LOG_FILE"
WRITE_EXIT=$?

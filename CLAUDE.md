...
- **The strip is one continuous wrap run, at every width.** `.map-filters { display: contents }` was a 1250px-only treatment, and having it apply only below that width was most of why the strip thrashed: above it, `.map-ranges`, `.map-filters` and `.map-ground` were three flex *items*, so each broke to a fresh line as a unit and an 814px chip group could not fit in the ~600px left beside the ranges. Dissolved, every chip, separator, range group and picker is an item in one run, so each line fills before the next begins. The chip gap therefore has to be declared on `.map-hud` — the box that used to supply it no longer generates a frame — and `.map-ranges` carries a `margin-right` because it is the one group with no `.map-filter-sep` to bound it. `.map-ground` lost `flex-basis: 100%` with the sentence: a picker and a 150px scale is ~300px and fits beside the time range.
- **The disclosure is at every width now, and says a different word on each side.** `key` on the desktop, where it reveals the legend; `layers` on the phone, where it also swallows the chips and the picker. Two label spans and two glyphs, one of each `display: none` per layout — so the hidden half leaves the accessibility tree with the pixels and the button announces one name. **Two boxes, because the two layouts fold at different depths**: `.map-hud-more` is the phone's panel and a `display: contents` pass-through on the desktop; `.map-hud-legend` is the desktop's panel and a pass-through on the phone, so the key and the note become rows of the phone's own panel rather than a bordered box inside a bordered box. `is-open` goes on both and each stylesheet block decides which one the word means. The legend hangs under the *strip* (`top: 100%`), never over a control, but is aligned to the button — `anchorLegend` measures, the way `--map-status-w` already does, because the button is the last item in a wrap run and lands anywhere; pinned to the HUD's right edge instead it opened 150px clear of the control that summoned it, with no horizontal overlap at all.
...
- Source: `public/islands/*.ts` — each entry exports `mount(container, props)`; shared utilities in `public/islands/_framework.ts` (Preact + `preact/hooks` + `htm` tagged templates, no compile step needed). **Not `@preact/signals`** — this line claimed it until 2026-08-01 and `_framework.ts` has never imported it; the package was a dependency nothing used, and `npm run deadcode` is what noticed.
...
`npm test` (= `node --test scripts/lib/*.test.js`) — corpus and log invariants. Each test pins a real bug the pipeline has had; baselines (e.g. known-bad cycle names, dup-pair counts) are observed values, not targets. If a test fails, read the diagnostic and fix the underlying issue — don't just raise the baseline to silence it.

## Static checking

`npm run verify` = `lint && typecheck && test`. Added 2026-08-01, because until
then the web surface had almost none of it: `tsconfig.islands.json` covered 38
files, **112 `.js` files under `scripts/`, `functions/` and `workers/` were
checked by nothing at all** — including `build.js`, which produces the site —
there was no linter at root, and CI ran one workflow that was
`workflow_dispatch`-only and scoped to `mobile/`.

- **`npm run lint` — Biome 2.5.5, linter only, `biome.jsonc`.** Already in the
  repo via `mobile/`, which pins the same version; `mobile/biome.json` carries
  `"root": false` so the two configs coexist and mobile's rules are untouched.
  It lints CSS and HTML as well as JS/TS, which is not incidental — the first
  thing it found was `.article-body` declaring `line-height` twice, the article
  reading surface setting at 1.6 while the rule said 1.75, invisible to
  `colour-system.test.js` because that only audits colour.
- **The formatter is off, deliberately.** This codebase is hand-aligned where
  the alignment is the argument — the vertex tables in `_map/glyphs.ts`, the
  ramp stops in `_map/style.ts`, the comment prose this file is made of. A
  format pass over 171 files is a diff nobody can review and a blame nobody can
  follow. `organizeImports` is off for the same reason.
- **`npm run typecheck` runs two projects.** `tsconfig.islands.json` (islands +
  `shared/`, `strict`) gained nine flags; seven cost zero errors, and
  `exactOptionalPropertyTypes` cost eleven. `isolatedModules` /
  `verbatimModuleSyntax` / `erasableSyntaxOnly` matter more than their zero cost
  suggests, since esbuild is the real compiler and transpiles per-file with no
  type information. **`noUncheckedIndexedAccess` is the one gap left**, measured
  at 96 errors concentrated in `_map/prayer.ts`, `shared/chart/series.ts` and
  `_map/places.ts` — the modules `map-geo.test.js` pins against `adhan` to
  within 20 seconds. It is the highest-value flag remaining and belongs in its
  own change. **`mobile/tsconfig.json` already sets it and already includes
  `../shared/**/*.ts`**, so `shared/chart/series.ts` is checked under it today
  and reports 18 errors — mobile's own `npm run typecheck` has been red on those
  the whole time. Landing the flag here closes both.
- **`tsconfig.node.json` is `allowJs` + `checkJs` with `strict` and
  `noImplicitAny` OFF, and that is the design.** These are untyped JS files; the
  value is in what TypeScript *infers* — wrong arity, a property that isn't on
  the shape, arithmetic on a boolean — not in extracting annotations from 20,000
  lines of working pipeline. `strict` would bury those under thousands of
  implicit-any reports and the gate would go unread. It found 232 errors and
  they are all fixed. The instructive ones were not crashes: `{object}` in JSDoc
  means TypeScript's *non-primitive with no properties*, so three `trends-expand`
  annotations documented nothing; an em-dash inside a `@param` is TS1127 and
  **silently discards the annotation**, which is why `og-image.js` had nine;
  `EntityRule` never declared `candidates` and `IndicatorDef` never declared
  `field` though both are read; and `MARKET_CATALOG`'s element type was whatever
  Tadawul happened to carry, so every `reason` on the twelve unavailable rows —
  the ones that file spends a paragraph justifying — was an unknown property.
- **`npm run deadcode` — knip, `knip.jsonc`.** Biome catches an unused variable
  inside a file; nothing caught an unused *file*, which is how five scripts
  totalling 812 lines sat in `scripts/` untouched since May. The config is
  almost entirely `entry`, and that is the shape of the problem: the pipeline
  scripts are invoked by name from a shell script, the islands are globbed by
  the bundler and fetched by string path at runtime, and `shared/` is imported
  by `mobile/`. **A script missing from that list is a script knip calls dead** —
  it did exactly that to `generate-edu-context.js`, which `run-replay.js` still
  runs via `spawnSync`. `shared/**` is declared `entry` rather than analysed,
  which means no coverage of its exports; adding mobile as a second workspace is
  what buys that back.
- **CI is `.github/workflows/checks.yml`**, on push and PR: lint, typecheck,
  deadcode, build. **`paths-ignore` for `content/**` is load-bearing** — the
  pipeline commits to master five times a day and every one of those commits
  touches only `content/`, so without it the workflow runs ~1,800 times a year
  to check nothing. **`npm test` is deliberately not in CI**: `logs.test.js`
  reads cycle logs out of gitignored `logs/` and `corpus.test.js` ratchets
  against the live corpus, neither of which exists on a runner.
- **The cycle's typecheck reports and never blocks.** `run-cycle.sh` runs it
  before the build behind a `timeout 120` and a `|| echo WARNING`. Everything
  from there to the deploy is written so build+commit+deploy runs even when an
  earlier stage times out — that is what stops a timeout becoming a no-publish
  cascade, and a checker must never be the thing that breaks it. The cost of
  that choice is a failure nobody reads, so `logs.test.js` ratchets on the
  warning at a baseline of zero: a type error is ours, and CI rejects it on push.
